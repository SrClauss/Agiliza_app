  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.token) {
        setAuth(data.token, data, 'PROFESSIONAL');
        router.push('/pro');
      } else {
    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        const res = await fetch('/api/auth/social/google', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id_token: idToken, target_role: 'PROFESSIONAL' }) // Changed role to target_role
        });
    ...
    <div style={{ 
      ...
      <div style={{ width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', gap: '28px' }}>
        ...
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
          <AgilizaLogo size={72} />
          <div>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'var(--md-sys-color-text)' }}>Painel Profissional</h1>
            <p style={{ color: 'var(--md-sys-color-text-muted)', fontSize: '0.9rem', marginTop: '2px' }}>Acesse suas oportunidades e clientes</p>
          </div>
        </div>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <Input 
                ...
                required
              />
              <Link href="#" style={{ fontSize: '0.8rem', color: 'var(--md-sys-color-text-muted)', alignSelf: 'flex-end', fontWeight: 500, marginTop: '2px' }}>
                Esqueceu a senha?
              </Link>
            </div>
        ...
        <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <p style={{ fontSize: '0.9rem', color: 'var(--md-sys-color-text-muted)' }}>
            Quer se tornar um parceiro? <Link href="/cadastro/profissional" style={{ color: 'var(--md-sys-color-primary)', fontWeight: 700 }}>Cadastre-se aqui</Link>
          </p>
          
          <Link href="/login/cliente" style={{ fontSize: '0.88rem', color: 'var(--md-sys-color-text-muted)', fontWeight: 600, display: 'inline-block', padding: '6px' }}>
            ← Sou um Cliente
          </Link>
        </div>
